...
// if you love cats join our community.
// https://t.me/bengalnetwork
// address Charity 0x2D22741F238A63cac8a7a62cF343B6144BE593CB
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xe2552BeD781AEAbb99A8A967081023124B8e160D;
...
contract BengalCat is Context, IERC20, Ownable {
    ...
    address private _burnAddress = 0x0000000000000000000000000000000000000000; //named burnfee now throughout. Dev->burn
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000000000* 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "BengalCat Token";
    ...
    uint256 private numTokensSellToAddToLiquidity = 1000000000000 * 10**18; //SF - Remove one 0 to make it 0.01% of supply instead of 0.1%
    ...
    function name() external view returns (string memory) { //SF - public->extenral where we can to save gas
        return _name;
    }
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 200); //SF - 20% max
        _taxFee = taxFee;
    }
    function setBurnFeePercent(uint256 BurnFee) external onlyOwner() {
        require(BurnFee <= 50); //SF - 5% max
        _BurnFee = BurnFee;
    }
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 50); //SF - 5% max
        _liquidityFee = liquidityFee;
    }
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent > 0); //SF - 1% minimum
    ...
    function removeAllFee() private { //SF Remove unneeded line
